# Tidy data for plotting {#sec-tidy}

![](images/banners/shape-of-your-data.png)

<!-- The "shape" wording is gone: the title, the {#sec-tidy} anchor, every
@sec-tidy reference and schedule.md all say tidy now. The filename and the
banner image are the last two, and they change the published URL, so they are
noted in class-notes/course-todo.md rather than done quietly. -->
...
```{r}
#| ref.label: ["pen-short-hidden", "pen-long-bill", "pen-two-val", "pen-year-wide"]
#| eval: false
#| echo: true
```
...
I wrote this one up on my blog, on baby name data rather than penguins:
[Just Quickly: Removing Sawtooth Patterns in Line
Graphs](https://www.njtierney.com/posts/2020-06-14-jq-sawtooth/).

It covers a third way this bites you, which we don't get to here: when your x
axis is a factor, ggplot2 quietly groups by x, and `group = 1` gives you a
zigzag that looks plausible and is wrong.
...
Once the data is tidy, the plot is still the bare minimum. Three
things make the penguin lines presentable.

```{r}
#| label: fig-polish
#| fig-alt: |
#|   Bill length across three years, one line per species, with a point at each
#|   observation and only the three real years on the x axis.
ggplot(pen_short,
       aes(x = year,
           y = bill_len,
           colour = species)) +
        geom_line() +
        geom_point(size = 2) +
        scale_x_continuous(breaks = unique(pen_short$year)) +
        labs(x = NULL,
             y = "Bill length (mm)",
             colour = NULL) +
        theme_minimal()
...
- `geom_point()` on top of the line, so you can see there are three
  observations and not a continuous measurement.
- `scale_x_continuous(breaks = ...)` because the default put 2007.5 on the axis,
  and there is no such year.
- The y label carries the unit. `bill_len` is a column name, millimetres is a
  measurement.
