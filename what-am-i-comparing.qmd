# Comparing groups {#sec-comparing}

![](images/banners/what-am-i-comparing.png)
...
This contains the parts from from @sec-tidy and @sec-computed:
...
:::{.callout-tip title="Going deeper: Change order to improve comprehension" collapse="true"}
...
We come back to ordering properly in @sec-colour.
...
**Tell it about the group.** `group = species` in @sec-tidy,
...
The dodged bars made the comparison. They are still hard to read a number off.

```{r}
#| label: fig-polish
#| fig-alt: |
#|   Weekday and weekend totals per sensor as dodged bars, sorted by total, with
#|   the axis in millions and the legend above the plot.
ggplot(pedestrian_totals,
       aes(x = total,
...
- **Sort the bars.** Alphabetical is not an order. This is @sec-colour's argument,
  and it costs one function.
- `label_number(scale = 1e-6, suffix = "M")` because nobody reads `7000000`.
- Drop the horizontal grid lines. On a bar chart they cross the bars and add
  nothing, since the bars already start from a common baseline.
